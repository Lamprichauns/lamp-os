...
class LampFadeIn(AnimatedBehaviour):
    async def draw(self):
        for j in range(self.lamp.base.num_pixels):
            self.lamp.base.buffer[j] = fade((0, 0, 0, 0), self.lamp.base.default_pixels[j], self.frame, self.frames)

        for j in range(self.lamp.shade.num_pixels):
            self.lamp.shade.buffer[j] = fade((0, 0, 0, 0), self.lamp.shade.default_pixels[j], self.frame, self.frames)
