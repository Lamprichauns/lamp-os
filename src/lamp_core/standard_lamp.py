...
from components.network.coding import Codes
...
class StandardLamp(Lamp):
    def __init__(self, name, base_color, shade_color):
        ...
        self.bluetooth = Bluetooth(self.network)
        self.network.announce_attribute(Codes.VERSION, 1)
        self.network.announce_attribute(Codes.BASE_COLOR, self.hex_to_rgbw(base_color))
        self.network.announce_attribute(Codes.SHADE_COLOR, self.hex_to_rgbw(shade_color))
        self.motion = MotionMPU6050(pin_sda=21, pin_scl=22)
        self.touch = Touch(pin=32)

        self.add_behaviour(StartNetworking)
